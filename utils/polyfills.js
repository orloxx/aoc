...
const POLYFILLS = {
  ...
  nMatrix() {
    Array.prototype.nMatrix = (n, fill = 0) => [...Array(n).keys()].fill(fill);
    assert.deepEqual([].nMatrix(2), [0, 0]);
    assert.deepEqual([].nMatrix(2, "n"), ["n", "n"]);
  },

  nm2DMatrix() {
    Array.prototype.nm2DMatrix = (n, m = n, fill = 0) =>
      Array.from({ length: n }, () => new Array(m).fill(fill));
  ...
  sumNMatrix() {
    Array.prototype.sumNMatrix = function (arr) {
      return this.map((num, idx) => num + arr[idx]);
    };
    assert.deepEqual([1, 2, 3].sumNMatrix([4, 5, 6]), [5, 7, 9]);
  },

  range() {
    Array.prototype.range = (n, m) => {
      if (!m) {
        return Array.from(new Array(n), (_, i) => i);
      }
      return Array.from(new Array(m - n), (_, i) => i + n);
    };
...
Object.keys(POLYFILLS).forEach((funcName) => {
  POLYFILLS[funcName]();
});
